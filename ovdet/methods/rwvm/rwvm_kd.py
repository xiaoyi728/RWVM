...
from .rwvm_base import RWVMBase
...
@OVD.register_module()
class RWVMKD(RWVMBase):
    def __init__(self, bag_weight, single_weight, individual_weight, use_attn_mask, bag_temp, single_temp, individual_temp,
                 use_gt,
                 clip_data_preprocessor,
                 boxes_cache=None,
                 **kwargs):
        super(RWVMKD, self).__init__(**kwargs)
        ...
        self.individual_temp=individual_temp # 70.0
    ...
    def _sample_on_topk_individual(self, topk_proposals):
        ...
        # 可能得提取这个box作为我们的image_encoder input
        boxes = nmsed_proposals.bboxes.tolist()
        # 执行bag 采样

        # ,metainfo=nmsed_proposals.metainfo.copy()看是否加上这句话
        sampled_instances = InstanceData(bboxes=nmsed_proposals.bboxes,metainfo=nmsed_proposals.metainfo.copy())
    ...
    def sample_individual(self, rpn_results, batch_data_sample, **kwargs):
        rpn_results.set_metainfo(batch_data_sample.metainfo)
        topk_proposals = self._sample_topk_proposals(rpn_results)
        if self.use_gt:
            topk_proposals = self._add_gt_boxes(topk_proposals,
                                                batch_data_sample.gt_instances.bboxes)
        sampling_result = self._sample_on_topk_individual(topk_proposals)
    ...
    def _bbox_clip_image(self, spanned_boxes, clip_images,
        ...
        # 这里直接获取sample_individual_list的bbox，和spanned-boxes维度对齐//应该有第二种方法 就是这个代码自己实现的
        # roi align本质上和roi_extractor一样
    ...
    def get_losses(self, pseudo_words, sampling_results, clip_model, images, indi_embedding,
                   transformer3, *args, **kwargs):

        image_ids = [res.img_id for res in sampling_results]
        device = pseudo_words.device
        # Note: perms = seq
        # 这里的normedbox变成了每张图片的box，经过sample之后的，
        ...
        normed_boxes_indiv=normed_boxes
        ...
        # 加上自注意力操作(多头)---- 残差连接--位置编码--局部
        pseudo_words = transformer3(pseudo_words, pos=position_embeddings )
        ...
        if self.single_weight > 0.0:
            # normbox是当前sample(bag之后的)之后的box的数量 得先获取
            # spanned box是进行组合的box数量
        ...
        if self.individual_weight > 0.0:
            preds_split_by_individual = [n.shape[0] for n in normed_boxes_indiv]
            img_ids = [torch.tensor(b * [img_id])
                       for b, img_id in zip(preds_split_by_individual,
                                            image_ids)]
            img_ids = torch.cat(img_ids).to(device)
            normed_boxes_indiv= torch.cat(normed_boxes_indiv, dim=0).split(preds_split_by_perms, dim=0)
            clip_indi_features = F.normalize(roi_align(
                clip_image_tokens, bbox2roi( normed_boxes_indiv).to(clip_image_tokens.dtype), (1, 1),
                float(clip_image_tokens.shape[-1]), 2, 'avg', True)[..., 0, 0], dim=-1)
            indi_embedding=F.normalize(indi_embedding)
            ...
            for res in sampling_results:
                for ori in res['box_ids']:
                    ...
                    box_ids.append(box_ids_per_ori)
            box_ids = torch.cat(box_ids).to(device)
            global_clip_indi_features = self.queues.get_queue('clip_indi_features')
            global_indi_embedding = self.queues.get_queue('indi_embedding')
            global_clip_indi_features_img_ids =  global_clip_indi_features[..., -1]
            global_indi_embedding_img_ids = global_indi_embedding[..., -1]
            num_queries = clip_indi_features.shape[0]
            assert num_queries == indi_embedding.shape[0]
            # text queue
            image_keys = torch.cat([clip_indi_features, global_clip_indi_features[..., :-1]])
            similarity_matrix_0 = self.individual_temp * indi_embedding @ image_keys.T
            if global_clip_indi_features_img_ids.shape[0] > 0:
                img_id_mask_0 = img_ids[:, None] == global_clip_indi_features_img_ids[None]
                similarity_matrix_0[:, num_queries:][img_id_mask_0] = float('-inf')
            # image queue
            text_keys = torch.cat([indi_embedding, global_indi_embedding[..., :-1]])
            similarity_matrix_1 = self.individual_temp * clip_indi_features @ text_keys.T
            if global_clip_indi_features_img_ids.shape[0] > 0:
                img_id_mask_1 = img_ids[:, None] == global_indi_embedding_img_ids[None]
                similarity_matrix_1[:, num_queries:][img_id_mask_1] = float('-inf')
            labels = torch.arange(num_queries, device=device)
            label_mask = box_ids[None] == box_ids[:, None]
            label_mask.fill_diagonal_(False)
            ...
            loss = F.cross_entropy(similarity_matrix_0, labels) * 0.5 \
                   + F.cross_entropy(similarity_matrix_1, labels) * 0.5
            # losses.update(loss_single=loss * self.single_weight)
            losses.update(loss_individual=loss * self.individual_weight)
            queues_update.update(indi_embedding=torch.cat([indi_embedding,
                                                               img_ids.view(-1, 1)], dim=-1).detach(),
                                 clip_indi_features=torch.cat([clip_indi_features,
                                                                img_ids.view(-1, 1)], dim=-1).detach())
