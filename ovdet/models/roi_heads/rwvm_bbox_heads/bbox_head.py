...
class RWVMBBoxHead(BBoxHead):
    def __init__(self,
                 ...
                 *args, **kwargs):
        super(RWVMBBoxHead, self).__init__(*args, **kwargs)
        ...
        cls_Linear_cfg_= self.cls_predictor_cfg.copy()
        cls_Linear_cfg_.update(
            in_features=self.in_channels, out_features=512)
        self.fc_cls = MODELS.build(cls_predictor_cfg_)
        self.fc_512_1 = MODELS.build(cls_Linear_cfg_)
    ...
    def vision_to_language(self, x):
        if self.with_avg_pool:
            if x.numel() > 0:
                ...
                # so use torch.mean instead it
                x = torch.mean(x, dim=(-1, -2))
        output=self.fc_cls(x).view(-1, self.num_words, self.word_dim)
        return output
